...
import { MODE_KEYS, DROPMENU_KEYS } from '@/components/OrderHistoryAdv/Constants';
...
class SettingsStore {
    ...
    @observable.shallow activeReports = [
    ...
    fetchPrice = async () => { // Todo: We will use FiatCurrencyStore instead of direct access here
    ...
    updateCurrencies = () => {
        ...
        if(this.countries) {
            this.countries.forEach(country => {
                const currency = currencies_country.find(currency => currency.currencyCode === country.currencyCode);
                if(!currency) {
                    ...
                    if(item) {
                        currencies_country.push({
                            ...
                            price: 1 / country.price,
                            symbol: item.symbol,
                            position: (country.currencyCode === 'USD' ? usdtValue : 0),
                            amountUsd: (country.currencyCode === 'USD' ? usdtValue : 0),
                        });
                    }
                }
            });
        ...
        const currencies_other = currencies_coin.concat(currencies_stock);
        currencies_other.sort((a, b) => {
            return (a.currencyCode.toLowerCase() < b.currencyCode.toLowerCase() ? -1 : 1);
        });
        this.currencies = currencies_country.concat(currencies_other);
    }
    ...
    loadLocalStorage = () => {
        ...
        this.activeReports = JSON.parse(localStorage.getItem('activeReports')) || DROPMENU_KEYS;
