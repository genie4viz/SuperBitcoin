...
const DEFAULT_FULL_DIGIT_LENGTH = 9;
...
export const customDigitFormatParts = (input, meta = {}) => {
    ...
    const digitGapReduction = meta.digitsGap >= DIGIT_GAP_THRESHOLD
        ? DEFAULT_DIGIT_GAP_REDUCTION
        : 0;
    ...
    switch (true) {
        ...
        case digitLength > 6:
            ...
            num /= 1000000;
            break;
        case digitLength > 3 && meta.digitsGap >= DIGIT_GAP_THRESHOLD:
            suffix = 'K';
            num /= 1000;
            break;
        default:
    ...
    const decimals = (digitLength <= 6 && meta.digitsGap < DIGIT_GAP_THRESHOLD)
        ? Math.min(Math.abs(DEFAULT_FULL_DIGIT_LENGTH - digitLength), meta.fractionDigits)
        : meta.fractionDigits;
    ...
    resultNumber = `${integerPart}.${fractionalPart}`;
...
export const highlightSearchDom = (src, search) => {
    ...
    for (let i = 0; i < positions.length; i++) {
        ...
        results.push(<span className="highlight" key={`${i}s`}>{src.substring(positions[i], positions[i] + search.length)}</span>);
