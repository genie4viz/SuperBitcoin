...
class Content extends Component {
    ...
    handleSubmitSecretCode = secretCode => () => {
        const {
            confirmAuthCode
        } = this.props[STORE_KEYS.SMSAUTHSTORE];
        ...
        if (secretCode.length === 4) {
            ...
            confirmAuthCode(secretCode, 'UpdateSecondFactor').then(() => {
                this.initData(true);
            }).catch(() => {
                this.setState({
                    isSecretCodeSent: false,
    ...
    initData(isReqestGoogleSecret = false) {
        ...
        this.setState({
            ...
            if (!isGoogle2FA && isReqestGoogleSecret) { // if currently the google 2fa is not enabled
                requestGoogleSecret().then(() => {
