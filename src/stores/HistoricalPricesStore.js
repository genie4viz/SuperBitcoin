...
class HistoricalPricesStore {
    ...
    constructor(orderBookStore) {
        ...
        reaction(
            () => ({
                ...
                multiLegMode: orderBookStore.multiLegMode,
            }),
            ({ base, quote, multiLegMode }) => {
                this.symbol = `${base}-${quote}`;
                this.multiLegMode = multiLegMode;
                this.multiLegPriceRate = orderBookStore.multiLegPriceRate;
    ...
    async fetchHistoricalPrices() {
        ...
        this.historicalData = data.map(item => ({
            x: item.time,
            y: this.multiLegMode
                ? 1 / (item.cls * this.multiLegPriceRate)
                : item.cls,
        }));
...
export default (orderBookStore) => new HistoricalPricesStore(orderBookStore);
