...
export const AniWalletButton = styled.div`
    ...
    justify-content: center;
    border: 2px solid ${({ type, masterColor, theme }) =>
        getThemeColor(type, masterColor, theme)};
    border-radius: 7px;
    border-right: 4px solid ${({ type, masterColor, theme }) =>
        getThemeColor(type, masterColor, theme)};
    outline: 2px dashed ${({ type, masterColor, theme }) =>
        getThemeColor(type, masterColor, theme)};
    outline-offset: -10px;
    padding: 5px 12px;
    ...
    > span {
        color: ${({ type, masterColor, theme }) =>
            getThemeColor(type, masterColor, theme)};
        background-image: ${props =>
            `linear-gradient(to ${props.isBuy ? "right" : "left"}, ${
                ...
                props.isBuy
                    ? props.theme.palette.btnPositiveBg
                    : props.theme.palette.btnNegativeBg
            } 66.66%, ${props.theme.palette.clrHighContrast} 66.66%)`};
