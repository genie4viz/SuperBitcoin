...
class FiatCurrencyStore {
    ...
    @action.bound getPriceOf = (symbol) => {
        return new Promise((resolve) => {
            if (this.stockMode && (symbol || '').includes('F:')) {
    ...
    fetchPrice = async () => { // Todo: Update - Remove this from FE
    ...
    @action.bound getStockPriceOf = (symbol) => { // Todo: Update - Remove this from FE
        return new Promise((resolve) => {
    ...
    @action.bound getSpotRate(base, quote) {
        let symbol = base;
        if (this.stockMode && (base || '').includes('F:')) {
            symbol = (base || '').replace('F:', 'S:');
        }
        return new Promise((resolve) => {
            fetch(`https://market-data.bct.trade/api/convert/price?fsym=${symbol}&tsym=${quote}`)
                .then(response => response.json())
                .then(data => {
                    resolve(data);
                })
