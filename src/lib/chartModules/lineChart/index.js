...
export default class LineChart {
    ...
    drawPriceLabel = (y, valueAtCursor) => {
        if (this.config.isPortfolio === true) {
            ...
            const symbol = this.config.coinSymbol;
            const currency = this.config.baseCoin;
            const isFiat = this.config.isFiat;
            let label = '';
            if (isFiat) {
                let fiatPrice = '';
                fiatPrice = valueAtCursor * (this.config.activeCoinValue ? this.config.activeCoinValue : 1);
                label = `1\u20BF = ${symbol} ${formatTotalDigitString(fiatPrice, 8)}`;
            } else {
                const price = valueAtCursor * (this.config.activeCoinValue ? this.config.activeCoinValue : 1);
                label = `1 ${currency} = ${symbol} ${formatTotalDigitString(price, 8)}`;
            }

            const labelWidth = ctx.measureText(label).width + 10;
            ctx.beginPath();
            ctx.moveTo(right, y - 10);
            ctx.lineTo(right - labelWidth, y - 10);
            ctx.lineTo(right - labelWidth, y + 10);
